...
bench_df, cpu_df, mem_df = load_python_results("results/results_2024-10-08_23-25-40/results.pickle")

external_groebner = load_external_results("results/results_2024-10-08_23-25-40/results.pickle")
...
res = []
gb = external_groebner.reset_index().groupby(by=["system"], observed=True)
for (k, df), m in zip(gb, gb.max(numeric_only=True).dropna()["timings"].values):
    df["timings"] /= m
    res.append(df)

external_groebner_normalised = pd.concat(res).set_index(["system", "library"]).sort_index()
del res
...
external_groebner_normalised_pivot = (
    external_groebner_normalised.groupby(["system", "library"], observed=False)
    .min(numeric_only=True)
    .dropna()
    .reset_index()
    .pivot(index="library", columns="system", values="timings")[order]
...
ax = sns.heatmap(
    pd.concat([groebner_normalised_pivot, external_groebner_normalised_pivot], axis=0),
    annot=True,
    fmt=".1f",
    vmin=0,
    vmax=(max_finite_normalised),
